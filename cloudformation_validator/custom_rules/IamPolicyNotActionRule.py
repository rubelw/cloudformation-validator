...
def lineno():
    ...
    return str(' -  IamPolicyNotActionRule- caller: '+str(inspect.stack()[1][3])+' - line number: '+str(inspect.currentframe().f_back.f_lineno))
...
class IamPolicyNotActionRule(BaseRule):
  ...
  def __init__(self, cfn_model=None, debug=None):
    """
    Initialize IamPolicyNotActionRule
    :param cfn_model: 
    """
  ...
  def rule_text(self):
    ...
    return 'IAM policy should not allow Allow+NotAction'
  ...
  def audit_impl(self):
    ...
    """
    if self.debug:
        print('IamPolicyNotActionRule - audit_impl'+lineno())
