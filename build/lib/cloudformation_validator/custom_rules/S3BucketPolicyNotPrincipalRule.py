...
def lineno():
    ...
    return str(' - S3BucketPolicyNotPrincipalRule - caller: '+str(inspect.stack()[1][3])+' - line number: '+str(inspect.currentframe().f_back.f_lineno))
...
class S3BucketPolicyNotPrincipalRule(BaseRule):
  ...
  def rule_text(self):
    ...
    return 'S3 Bucket policy should not allow Allow+NotPrincipal'
  ...
  def audit_impl(self):
    ...
    """
    if self.debug:
      print('S3BucketPolicyNotPrincipalRule - audit_impl'+lineno())
